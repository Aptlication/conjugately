...
- **Recent Fixes (January 2025)**: Fixed missing Present Progressive (Présent Progressif) quizzes for reflexive verbs (se lever, s'appeler, se sentir) through improved tense normalization mapping. Resolved specific permutation issues for dire/voir Imparfait and Plus-que-parfait tenses. Enhanced grammar conversion to properly handle "will don't" and "had don't" constructions with better auxiliary vs main verb detection. Fixed "used to don't say" grammar errors in imparfait tense by adding comprehensive negation patterns for all verbs (January 10, 2025). Restored complete Advanced level with full set of 13 verbs including all reflexive verbs (se lever, s'appeler, se sentir, se laver, se réveiller, s'habiller) plus non-reflexive verbs (être, avoir, faire, dire, aller, voir, savoir, pouvoir, vouloir, venir) - January 11, 2025. Implemented tiered selection system: only Advanced level shows specific tense selection, while Novice/Easy/Moderate levels automatically map tenses based on time frame. Advanced level fully unlocked and accessible without prerequisites. Updated Novice past tense mapping from Passé Simple to Passé Composé for consistency across all basic levels. Unified difficulty level descriptions: Novice, Easy, and Moderate now all show "Present, past, and future tenses" while Advanced shows "All tenses and time frames" for consistency across both modal and dropdown interfaces. **COMPLETED**: Fixed present tense consistency - all Novice/Easy/Moderate levels now use only "Présent" (present simple) instead of "Présent Progressif" for present time frame. System verified working correctly with workflow restart clearing cached state (January 11, 2025). **COMPLETED**: Fixed English grammar errors in quiz questions by enhancing the `fixEnglishGrammar` function to properly handle third-person singular forms of "make" and "have" verbs, preventing errors like "She make mistakes" (January 11, 2025). **COMPLETED**: Enhanced pedagogical approach for present tense questions - all QUESTION_CONTEXTS now show dual English forms like "I eat / I am eating" to teach students that French "Je mange" covers both meanings, demonstrating the efficiency of French present tense over English (January 11, 2025). **COMPLETED**: Implemented comprehensive past tense progressive form removal system for all non-Advanced levels across ALL major past tenses (Passé Composé, Imparfait, Plus-que-parfait, Passé Simple). Enhanced pedagogical approach where basic levels (Novice/Easy/Moderate) show simplified past forms like "I knew", "I used to do" while Advanced level preserves advanced dual forms like "I knew / I was knowing". Added robust regex-based filtering with comprehensive coverage of all progressive patterns including "was/were + ing", "had been + ing", and complex negation forms. All quiz types automatically inherit enhancements through unified QUESTION_CONTEXTS system (January 11, 2025). **COMPLETED**: Applied comprehensive French grammar corrections to novice level content via Perplexity AI verification (January 12, 2025) - Added proper gender agreement for all adjectives (heureux/heureuse, fatigué/fatiguée, occupé/occupée, prêt/prête), implemented masculine/feminine forms for nous/ils/elles contexts, enhanced quiz templates with gender-specific questions and rationales, corrected pronunciation notes, and updated context sentences to include proper gender distinctions. All novice level être, avoir, faire content now follows authentic French grammar rules with comprehensive gender agreement system. **COMPLETED**: Systematic rebranding from "Beginner" to "Novice" across all application components (January 13, 2025) - Updated client-side components, server-side logic, database references, and documentation to consistently use "Novice" as the entry-level difficulty, maintaining pedagogical progression and user experience continuity. **COMPLETED**: Enhanced être present tense questions for both Beginner and Novice levels (January 22, 2025) - Implemented contextual questions like "I am happy", "You are tired", "He is a student", etc., replacing simple pronoun conjugations. Both levels now feature comprehensive educational content with detailed rationales explaining proper French conjugation rules, negation patterns (ne...pas), and common conjugation errors. Added complete future tense dataset for faire verb in Beginner level with 20 questions covering affirmative, negative, and interrogative forms. **COMPLETED**: Comprehensive 5-tier difficulty system implementation (January 13, 2025) - Added new "Beginner" level above Novice focusing on subject pronoun conjugations (Je suis, Tu es, Il/Elle est, etc.) using provided Python curriculum. Implemented systematic rebranding throughout entire application: Easy→Elementary, Moderate→Intermediate, Difficult→Advanced. Updated all client-side components (App.tsx, fresh-app.tsx, FrenchQuiz.tsx), server-side logic (quiz-generator.ts, beginner-pronoun-data.ts), and documentation to support new 5-tier progressive system: Beginner→Novice→Elementary→Intermediate→Advanced with increasing verb sets (3→3→6→8→13 verbs) for optimal learning progression. **MAJOR MILESTONE COMPLETED**: Intermediate Level 100% Complete (January 13, 2025) - Successfully implemented all 11 verbs × 4 tenses × 20 questions = 880 total MCQs for Intermediate level. Complete verb set: être, avoir, faire, aller, voir, dire, pouvoir, vouloir, prendre, venir, savoir. All four tenses implemented: Présent, Passé Composé, Futur Simple, Imparfait. Updated UI configuration to reflect complete 11-verb system with corrected difficulty descriptions and course structure. Intermediate level now provides comprehensive intermediate-style full sentence questions with authentic French grammar and systematic block organization. **CRITICAL FIX COMPLETED**: Resolved quiz looping issue (January 14, 2025) - Fixed data structure mismatch between frontend expecting `options` array with `correctAnswer` format and server providing `answerOptions` array with `{text, rationale, isCorrect}` objects. Updated frontend to properly handle server's `answerOptions` structure, including quiz display, answer confirmation logic, results calculation, and detailed review section. Applied aggressive cache-busting to resolve browser caching issues. Quiz functionality now works correctly without looping back to configuration screen.

### Cloud Text-to-Speech Integration (ElevenLabs)
- **Service Architecture**: REST API integration with ElevenLabs TTS using server-side memoization caching (100 phrases, 24-hour TTL) via `server/services/elevenlabs.ts`.
- **API Endpoint**: `POST /api/tts` accepts `{text, voiceId?}` and returns binary audio/mpeg data with proper caching headers.
- **Frontend Hook**: `useTTS` hook (`client/src/hooks/useTTS.ts`) provides `speakAnswer()` function that tries cloud TTS first, automatically falls back to browser TTS if cloud fails.
- **iOS Safari Compatibility**: Uses hidden `<audio playsInline>` element with ref callback pattern for reliable playback on mobile Safari.
- **Memory Management**: Blob URL cleanup on unmount, error, and before each new audio load to prevent memory leaks.
- **Feature Flag**: `cloudTTSEnabled` in `shared/config.ts` allows instant rollback to browser-only TTS if needed.
- **Voice Selection**: Default voice is Léa (EXAVITQu4vr4xnSDxMaL) - native French female speaker using multilingual_v2 model for authentic pronunciation.
- **Fallback Strategy**: If cloud TTS fails (network error, API issue, or no audio element), browser's native French voice (Google français) is used automatically.

### Advanced Difficulty Lock System (Version 1 Strategic Deployment)
- **Feature Management**: Centralized configuration-driven approach using `shared/config.ts` with `ADVANCED_DIFFICULTY_ENABLED = false` for Version 1 deployment strategy.
- **Frontend Lock Implementation**: Advanced difficulty cards display lock icons and disabled states in App.tsx, with dropdown option showing "🔒 Advanced - Coming Soon!" instead of full description.
- **User Experience**: Professional informational modals explaining "Advanced level coming in future version!" with compelling feature previews (13 essential verbs, all tenses, reflexive conjugations, professional-level fluency).
- **Backend Security**: Server-side validation in routes.ts prevents Advanced difficulty requests with 403 status and "ADVANCED_LOCKED" error code for security consistency.
- **Unlock Mechanism**: Simple one-line change in `shared/config.ts` from `false` to `true` enables full Advanced functionality across entire application without code modifications.
- **Strategic Purpose**: Sets clear user expectations for future releases while maintaining professional appearance, avoiding feature hiding that could confuse users about platform capabilities.
- **Deployment Benefits**: Allows Version 1 to focus on perfecting Beginner through Intermediate levels while Advanced remains visible as a "coming soon" feature, building anticipation for future releases.
