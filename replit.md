...
- **Course System**: Progressive difficulty system with logical unlocking (Novice→Easy→Moderate→Advanced), where Advanced level only appears after completing any Moderate course exam. Structured learning flow with sequential progression (Present → Past → Future tenses), tiered difficulty levels featuring increasing verb sets (3→6→8→13 verbs), and comprehensive grammar fixes for English negation patterns. All basic levels (Novice, Easy, Moderate) use simple tenses only (Présent, Passé Composé, Futur Simple/Proche), while Advanced level features the complete set of reflexive and non-reflexive verbs with all tenses including progressive forms.
