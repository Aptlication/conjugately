...
- **Cloud Text-to-Speech Integration (ElevenLabs)**: REST API integration with server-side memoization caching. Frontend `useTTS` hook provides `speakAnswer()` (no browser TTS fallback — silence if MP3 missing). Supports iOS Safari. Voice assignments: Léa (`EXAVITQu4vr4xnSDxMaL`) for Beginner (1,697 answer phrases + 240 question MP3s), Thomas (`GBv7mTt0atIp3Br8iCZE`) for Novice (922 answer phrases + 240 question MP3s), Audrey (`McVZB9hVxVSk3Equu8EH`) for Elementary (1,757 answer phrases + 504 question MP3s). Manifest at `attached_assets/tts-manifest.json` with `phrases`, `novice_phrases`, and `elementary_phrases` sections. Question audio path: `/attached_assets/audio/quizzes/{difficulty}/{verb}/{tensePath}/questions/Q{audioIndex}.mp3`.
