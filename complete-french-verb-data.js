// French Verb Master - Complete Data Export for React Native Mobile App
// Comprehensive verb conjugation database with 13 verbs across 9 tenses
// Includes 4-tier difficulty system, question contexts, and helper functions

export const FRENCH_VERB_DATA = {
  // Complete verb conjugations - 13 verbs × 9 tenses each
  VERB_CONJUGATIONS: {
    être: {
      présent: {
  ...
  // 4-Tier Difficulty System with Course Structure
  DIFFICULTY_CONFIGS: {
    "Beginner": { 
    ...
    "Moderate": { 
      ...
      courseStructure: {
        ...
        finalExam: { 
          questions: 80, 
          questionsPerVerb: 10, 
          passThreshold: 72, // 90% pass rate
          description: "10 questions from each of the 8 verbs"
        }
      }
    },
    "Difficult": { 
      verbs: ["être", "avoir", "faire", "dire", "aller", "voir", "savoir", "pouvoir", "vouloir", "venir", "se laver", "se réveiller", "s'habiller"], // 13 verbs total
      ...
      courseStructure: {
        ...
        finalExam: { 
          questions: 130, 
          questionsPerVerb: 10, 
          passThreshold: 117, // 90% pass rate
  ...
  // Question contexts with English sentences and French contexts (30% negation coverage)
  QUESTION_CONTEXTS: {
    être: [
    ...
    avoir: [
      ...
      { en: "She doesn't have time.", fr_context: "de temps", negative: true, pronoun: "elle" },
      { en: "We don't have money.", fr_context: "d'argent", negative: true, pronoun: "nous" }
    ],
    faire: [
      ...
      { en: "She doesn't make mistakes.", fr_context: "d'erreurs", negative: true, pronoun: "elle" },
      { en: "We don't do that.", fr_context: "ça", negative: true, pronoun: "nous" }
    ],
    voir: [
    ...
    dire: [
      ...
      { en: "She doesn't say that.", fr_context: "ça", negative: true, pronoun: "elle" },
      { en: "We don't say bad words.", fr_context: "de gros mots", negative: true, pronoun: "nous" }
    ],
    savoir: [
      ...
      { en: "We don't know where.", fr_context: "où", negative: true, pronoun: "nous" }
    ],
    pouvoir: [
      ...
      { en: "I can't sleep.", fr_context: "dormir", negative: true, pronoun: "je" },
      { en: "She can't come.", fr_context: "venir", negative: true, pronoun: "elle" },
      { en: "We can't wait.", fr_context: "attendre", negative: true, pronoun: "nous" }
    ],
    vouloir: [
      ...
      { en: "She doesn't want to go.", fr_context: "partir", negative: true, pronoun: "elle" },
      { en: "We don't want problems.", fr_context: "de problèmes", negative: true, pronoun: "nous" }
    ],
    venir: [
      ...
      { en: "I don't come often.", fr_context: "souvent", negative: true, pronoun: "je" },
      { en: "She doesn't come today.", fr_context: "aujourd'hui", negative: true, pronoun: "elle" },
      { en: "We don't come here.", fr_context: "ici", negative: true, pronoun: "nous" }
    ],
    aller: [
    ...
    "se lever": [
      ...
      { en: "I don't get up early.", fr_context: "tôt", negative: true, pronoun: "je" },
      { en: "She doesn't get up easily.", fr_context: "facilement", negative: true, pronoun: "elle" },
      { en: "We don't get up late.", fr_context: "tard", negative: true, pronoun: "nous" }
    ],
    "s'appeler": [
      ...
      { en: "I am not called that.", fr_context: "comme ça", negative: true, pronoun: "je" },
      { en: "She is not called Marie.", fr_context: "Marie", negative: true, pronoun: "elle" },
      { en: "We are not called beginners.", fr_context: "débutants", negative: true, pronoun: "nous" }
    ],
    "se sentir": [
      ...
      { en: "I don't feel well.", fr_context: "bien", negative: true, pronoun: "je" },
      { en: "She doesn't feel safe.", fr_context: "en sécurité", negative: true, pronoun: "elle" },
      { en: "We don't feel comfortable.", fr_context: "à l'aise", negative: true, pronoun: "nous" }
    ],
    "se laver": [
      ...
      { en: "I don't wash often.", fr_context: "souvent", negative: true, pronoun: "je" },
      { en: "She doesn't wash well.", fr_context: "bien", negative: true, pronoun: "elle" },
      { en: "We don't wash there.", fr_context: "là", negative: true, pronoun: "nous" }
    ],
    "se réveiller": [
      ...
      { en: "I don't wake up easily.", fr_context: "facilement", negative: true, pronoun: "je" },
      { en: "She doesn't wake up early.", fr_context: "tôt", negative: true, pronoun: "elle" },
      { en: "We don't wake up late.", fr_context: "tard", negative: true, pronoun: "nous" }
    ],
    "s'habiller": [
      ...
      { en: "I don't dress up.", fr_context: "chic", negative: true, pronoun: "je" },
      { en: "She doesn't dress warmly.", fr_context: "chaudement", negative: true, pronoun: "elle" },
      { en: "We don't dress alike.", fr_context: "pareil", negative: true, pronoun: "nous" }
    ]
  }
};

// Helper functions for React Native implementation
export const FRENCH_VERB_HELPERS = {
  // Get verbs for specific difficulty level
  getVerbsForDifficulty: (difficulty) => {
    return FRENCH_VERB_DATA.DIFFICULTY_CONFIGS[difficulty]?.verbs || [];
  },

  // Get tenses for specific difficulty level
  getTensesForDifficulty: (difficulty) => {
    return FRENCH_VERB_DATA.DIFFICULTY_CONFIGS[difficulty]?.tenses || [];
  },

  // Get course structure for difficulty level
  getCourseStructure: (difficulty) => {
    return FRENCH_VERB_DATA.DIFFICULTY_CONFIGS[difficulty]?.courseStructure || null;
  },

  // Get conjugation for specific verb, tense, and pronoun
  getConjugation: (verb, tense, pronoun) => {
    return FRENCH_VERB_DATA.VERB_CONJUGATIONS[verb]?.[tense]?.[pronoun] || null;
  },

  // Get question contexts for specific verb
  getQuestionContexts: (verb) => {
    return FRENCH_VERB_DATA.QUESTION_CONTEXTS[verb] || [];
  },

  // Get verb meaning in English
  getVerbMeaning: (verb) => {
    return FRENCH_VERB_DATA.VERB_MEANINGS[verb] || verb;
  },

  // Check if verb is reflexive
  isReflexiveVerb: (verb) => {
    return verb.startsWith('se ') || verb.startsWith("s'");
  },

  // Get all available verbs
  getAllVerbs: () => {
    return Object.keys(FRENCH_VERB_DATA.VERB_CONJUGATIONS);
  },

  // Get all available tenses
  getAllTenses: () => {
    return Object.keys(FRENCH_VERB_DATA.VERB_CONJUGATIONS.être);
  },

  // Generate a random quiz question
  generateQuizQuestion: (verb, tense, difficulty = 'Easy') => {
    const contexts = FRENCH_VERB_HELPERS.getQuestionContexts(verb);
    if (contexts.length === 0) return null;
    
    const randomContext = contexts[Math.floor(Math.random() * contexts.length)];
    const conjugation = FRENCH_VERB_HELPERS.getConjugation(verb, tense, randomContext.pronoun);
    
    if (!conjugation) return null;
    
    return {
      english: randomContext.en,
      frenchContext: randomContext.fr_context,
      pronoun: randomContext.pronoun,
      correctAnswer: conjugation,
      verb: verb,
      tense: tense,
      isNegative: randomContext.negative || false
    };
  },

  // Get random verb from difficulty level
  getRandomVerbForDifficulty: (difficulty) => {
    const verbs = FRENCH_VERB_HELPERS.getVerbsForDifficulty(difficulty);
    return verbs[Math.floor(Math.random() * verbs.length)];
  },

  // Get random tense from difficulty level
  getRandomTenseForDifficulty: (difficulty) => {
    const tenses = FRENCH_VERB_HELPERS.getTensesForDifficulty(difficulty);
